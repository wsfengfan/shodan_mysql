...
echo \"telnet_status=\"`chkconfig --list |egrep \"*.telnet\"|egrep -i \"on|启用\"|wc -l`
...
unset ssh_status;
";
push(@array_pre_flag, 19);$pre_cmd{20} = "cat /etc/shadow|sed '/^\\s*#/d'|awk -F: '(\$2!~/^*/) && (\$2!~/^!!/) {print \$1\":\"}'|egrep \"^lp:|^sync:|^halt:|^news:|^uucp:|^operator:|^games:|^gopher:|^smmsp:|^nfsnobody:|^nobody:\"
...
";
push(@array_pre_flag, 4597);$pre_cmd{-11} = "hostname
";
